...
define(function (require, exports, module) {
  ...
  function Context(id) {
    ...
    function setupResources(available) {
      _available = [];
      ...
      // return the first locale on the fallback chain
      return _locales[_available[0]];
    }

    function dummy() {
      var defer = new Promise();
      defer.fulfill(_registered);
      return defer;
    }

    function freeze(negotiate) {
      if (_isFrozen && !_isReady) {
        throw new ContextError('Context not ready');
      }
      ...
      (negotiate || dummy)(_registered)
        ...
        // emit any known errors
        .then(setReady, echo.bind(null, 'error'))
        // emit unknown errors from callbacks as 'debug'
